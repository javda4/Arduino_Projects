...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // If the relay state has changed, log the change
    if (newRelayState != relayState) {
      relayState = newRelayState;
      digitalWrite(RELAY_PIN, relayState ? HIGH : LOW);
      logRelayChange(relayState, false); // Change due to app, not manual override
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    if (!buttonPressed) {
      ...
      logRelayChange(relayState, true); // Change due to manual override
      displayText(relayState ? "Relay ON" : "Relay OFF");
    }
  } else {
    buttonPressed = false;
